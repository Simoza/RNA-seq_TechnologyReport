...
Differential gene expression analysis consists in taking the normalized read count data and performing statistical analysis to discover quantitative changes in expression levels between experimental groups. To describe the workflow I will use RNA-seq data that I collected from chemostats experiments. Briefly, we performed several chemostat runs to study differences in phenotype related to four factors: 

1. **Strain**: we tested three *Y. lipolytica* strains, one wild-type (OKYL029), one lipid accumulator (OKYL049), and a lipid-free strain (JFYL007).

2. **C/N ratio**: *Y.lipolytica* accumulates lipids when the nitrogen becomes limiting (high carbon-to-nitrogen ratio). We tested a high (116) and a low (3) C/N ratio) to assess differences in cell phenotype, organization, metabolism, and transcriptome related to the lipid accumulation capacity of different strains.

3. **Dilution rate**: the dilution rate expresses the relation between the flow of medium into the bioreactor and the culture volume within it. In a chemostat cultivation the volume is kept constant, and the dilution rate influences the rate to which cells get fresh media, in turn influencing the growth rate. We tested two dilution rates, 0.06, in which the flow of fresh media is low and results in lower growth rate, and 0.1, in which the media is fed at a higher rate, resulting in higher growth rates. 

4. Nitrogen source: ammonium sulphate is a widely used nitrogen source for the cultivation of microorganisms; urea is a sustainable and cheap alternative nitrogen source. We investigated the influence of urea as a nitrogen source compared to ammonium sulphate.

For each condition we ran quadruplicates and sent triplicates for RNA-seq. For simplicity the workflow presented only takes the strain factor into account, but the analysis can easily be extended to other factors. 
...
* The function accesses the group factor contained in the DGEList (x\$samples\$group) in order to compute the minimum group size, but the filtering is performed independently of which sample belongs to which group so that no bias is introduced. In our case, x\$samples\$group is a factor with 15 levels, and each levels contains three samples, so the minimum number of samples is three since the smallest group size is three.

*After determining the minimum group size, the function calculates the median library size (median(x\$lib.size)) and uses that value to determine a CPM.Cutoff. The CPM.Cutoff is defined as the ratio between the min.count (default at 10) adjusted to 10<sup>6</sup> to convert to count-per-million, and the median library size. 

* Calculated the CPM.Cutoff, the function only keeps those genes for which at least 3 samples (minimum group size) have a CPM value higher than the CPM-Cutoff.
...
```{r, results='hide'}
summary(decideTests(vfit, adjust.method = "BH", p.value = 0.01, lfc = 1))

```

```{r, echo = FALSE}
t <- summary(decideTests(vfit, adjust.method = "BH", p.value = 0.01, lfc = 1))
...
```{r, figures-side, fig.show="hold", out.width="50%", fig.show='hide', warning=FALSE}
...
for (i in 1:length(GSA_results)) { #loop through all the GSAs we have
  ...
  names(consGSA_plot_data)[i] <- names(GSA_results)[i] #assign the names to the list of graphs
    ...
    print(ggarrange(consGSA_plot_data[[i]][["BP"]][["plot"]], 
                        ...
                        common.legend = TRUE, 
                        legend = "bottom") %>%
      annotate_figure(top = text_grob((names(comparison_list)[i]), 
